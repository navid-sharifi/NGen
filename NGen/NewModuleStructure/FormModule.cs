...
"));
        ...
        public override string GetReactPage(Type pageType, Type moduleType)
        {
            foreach (var item in Fields)
            {
                ...
                var imports = item.ReactImports();
                if (imports.Any())
                    this.ReactImport.AddRange(imports);
...
export default {GetReactModuleName(pageType, moduleType)};";
    ...
    public class FormModuleField
    {
        ...
        public IEnumerable<CustomAttributeData> Attributes;
        ...
        public Func<Type,Type,string,string> ReactHtmlFunc { get; set; } = null;

        internal string ReactHtml(Type pageType, Type moduleType, string viewModel)
        {

            if (ReactHtmlFunc != null)
                return ReactHtmlFunc.Invoke(pageType, moduleType, viewModel);
...
";
        ...
        public List<(string Name, string Import)> ExtraReactImports { get; set; } = new List<(string Name, string Import)>();

        public List<(string Name, string Import)> ReactImports()
        {
            var list = new List<(string Name, string Import)>();
            list.AddRange(ExtraReactImports);
            return list;
        }
